...
uses gw.api.databuilder.TransToClaimExchangeRateInUseBuilder
...
class SampleClaimsForExistingPoliciesInPCForCompany extends SampleDataBase {
  ...
  private function findPolicy(policyStatus: PolicyStatus, criteria: PolicySearchCriteria, lossDate: Date): Policy {
    ...
    var result : PolicySearchResultSet = null
    try {
      result = searcher.searchPolicies(criteria)
    } catch ( e : Exception ) {
      return null
    }
    var policyType = criteria.PolicyType
    var summary = result.Summaries.firstWhere(\s -> s.Status == policyStatus and (policyType == null or s.PolicyType == policyType))
    if (summary != null) {
  ...
  private function createOpenCommercialPropertyClaimWithFinancials(bundle: Bundle) {
    ...
    new ReserveBuilder()
        ...
        .onClaim(claim)
        .withTransToClaimExchangeRateFromEither(
            \bldr-> getExchangeRateValueGenerator(Currency.TC_USD, Currency.TC_USD),
            \bldr -> getFXRateValueGenerator(bldr, Currency.TC_USD, Currency.TC_USD))

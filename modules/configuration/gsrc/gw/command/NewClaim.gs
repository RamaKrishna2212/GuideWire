...
class NewClaim extends CCBaseCommand {
  ...
  @Argument("PolicyType", "PersonalAuto")
  function draftWithPolicyType() {
    final var policyType = getArgument("PolicyType")
    var claim = ClaimBuilder.uiReady(policyType)
        .withNonConflictingClaimNumber()
        .withAppropriateIncident(policyType)
        .withState(ClaimState.TC_DRAFT)
        .createAndCommit()

    CCLoggerCategory.TEST.info("** Created claim " + claim.ClaimNumber)
    final var location = FNOLWizard.go(claim) as Wizard
    location.next()
    location.next()
    location.next()
  }
  ...
  function hopHomeowners() {
    var claim = ClaimBuilder.uiReadyHOPHomeowners()
        .withNonConflictingClaimNumber()
        .open()
        .assigned()
  ...
  function catHOPClaims() {
    ...
    for (address in addresses) {
      gw.transaction.Transaction.runWithNewBundle( \ claimBundle -> {
        ...
        var claim = setClaimFields(new ClaimBuilder().uiReadyHOPHomeowners(), "Midwest Property A", "wdufraine",
    ...
    var sensitiveClaim = setClaimFields( new ClaimBuilder().uiReadyHOPHomeowners(),
...
//        print("Creating policy # ${pcPolicyNumber}")
        ...
        gw.transaction.Transaction.runWithNewBundle( \ claimBundle -> {
          ...
          claim = setClaimFields(new ClaimBuilder().uiReadyHOPHomeowners(), adjuster[1], adjuster[0], cat,
              LossCause.TC_WATERDAMAGE, LossType.TC_PR, address)
              .withPolicy(PolicyBuilder.hopHomeowners()
          ...
          claim.Insured.PrimaryAddress = buildAddress(address).create()

          var exposure = ExposureBuilder.uiReadyHOPDwelling()
              .onClaim(claim)
              .withClaimAssignedUser()
              .withClaimAssignedGroup()
              .create(claimBundle)
        ...
        if (generatePolicyLocations) {
          createHOPPolicyLocationSummary(catastrophe, insured[0] + " " + insured[1], address, claim, pcPolicyNumber)
        }
...
//          print("Creating policy # ${pcPolicyNumber}")
          policyCounter++
          createHOPPolicyLocationSummary(catastrophe, insured[0] + " " + insured[1], address, null, pcPolicyNumber)
        }
      }
      ...
      for (address in closedAddresses) {
        gw.transaction.Transaction.runWithNewBundle( \ claimBundle -> {
          ...
          var claim = setClaimFields(new ClaimBuilder().uiReadyHOPHomeowners(),
  ...
  // Create a policy location summary for the address.  If claim is non-null, make fields match the claim
  function createHOPPolicyLocationSummary(catastrophe: Catastrophe, name: String, address: java.util.ArrayList<java.lang.String>, claim: Claim, pcPolicyNumber: String) {
    gw.transaction.Transaction.runWithNewBundle(\plsBundle -> {
      var policyLocationSummary = new PolicyLocationSummaryBuilder()
          .withCatastrophe(catastrophe)
          .withAddressLine1(address[0])
  ...
  function withClaimIndicators() : String {
    ...
    newClaim.CoverageInQuestion = true
  ...
  function withCheck() {
    ...
    pcf.ClaimSummary.go( claim )
    claim.Exposures.first().setAvailableReserves(costType, costCategory , (Coercions.makeBigDecimalFrom(Argument)) *2, User.util.CurrentUser)
    claim.Exposures.first().newCheckCreator()
  ...
  function withRecovery() {
    ...
    pcf.ClaimSummary.go( claim )
    claim.Exposures.first().setAvailableReserves(costType, costCategory , (Coercions.makeBigDecimalFrom(Argument)) *2, User.util.CurrentUser)
    var chk = claim.Exposures.first().newCheckCreator()
  ...
  @Argument("Policy Type", "PersonalAuto")
  @Argument("Incident Type", "vehicle")
  @Argument("Loss Cause", "vehcollision")
  function withIncident() {
    if (PolicyType.TC_WORKERSCOMP.Code.equalsIgnoreCase(getArgument("Policy Type"))) {
      displayMessageAndExit(
          String.format("Creation of policy type: %s is not supported.", {getArgument("Policy Type")}))
    }

    var policyBuilder = new PolicyBuilder()
        .withPolicyType(PolicyType.getTypeKey(getArgument("Policy Type")))
        .withNewPersonInsured(false)
        .effectiveNow()
    ...
    switch (getArgument("Incident Type").toLowerCase()) {
      ...
      case "trip":
        ...
        policyBuilder.withRiskUnit(new TripRUBuilder()
            .withRUNumber(1)
        ...
        break
      case "vehicle":
        incidentBuilder = VehicleIncidentBuilder.uiReadyVehicleIncident().withVehicleLossParty(LossPartyType.TC_INSURED)
        break
      default:
        displayMessageAndExit(
    ...
    var claim = new ClaimBuilder()
        .uiReady()
        .withPolicy(policyBuilder)
        .withLossType(CCDependencies.getBusinessLineConfiguration()
            .getLossTypesForSpecifiedPolicyType(PolicyType.getTypeKey(getArgument("Policy Type")))
            .first())
        .withLossCause(LossCause.getTypeKey(getArgument("Loss Cause")))
        .withIncident(incidentBuilder)
        .createAndCommit()

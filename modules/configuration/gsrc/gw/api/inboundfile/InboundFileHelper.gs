...
uses jakarta.activation.MimetypesFileTypeMap
...
class InboundFileHelper {
  ...
  static function download(archiveLocation : String) : void {
    var file = new File(archiveLocation);
    var fileType = new MimetypesFileTypeMap().getContentType(archiveLocation)
    WebFileUtil.copyFileToClient(file, file.getName(), fileType);
  }

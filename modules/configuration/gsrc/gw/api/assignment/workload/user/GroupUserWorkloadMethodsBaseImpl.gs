  ...
  public override function calculateWorkload() : int {
    ...
    if (CCLoggerCategory.ASSIGNMENT_WEIGHTED_WORKLOAD.TraceEnabled) {
      if (not workloadMatrix.Empty) {
        ...
        CCLoggerCategory.ASSIGNMENT_WEIGHTED_WORKLOAD.trace("'" + Owner.Group.DisplayName + " - " + Owner.User.LogDisplayName + "' has weight " + result)
      } else {
        CCLoggerCategory.ASSIGNMENT_WEIGHTED_WORKLOAD.trace("'" + Owner.Group.DisplayName + " - " + Owner.User.LogDisplayName + "' has no weight")
      }
    }

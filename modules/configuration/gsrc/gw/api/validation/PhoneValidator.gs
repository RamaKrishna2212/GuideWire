...
class PhoneValidator extends FieldValidatorBase {
  ...
  private var strict = PLConfigParameters.PhoneNumberStrictValidation.getValue()
  ...
  override function validate(phoneNumber : String, p1 : Object, parameters : Map<Object, Object>) : IFieldValidationResult {
    ...
    if (extensionProperty != null) {
      extension = p1[extensionProperty] as String
    ...
    if (phoneCountry != null
      ...
      if (!PhoneUtil.isViableNumber(phoneNumberOriginal)) {
        result.addError(DisplayKey.get("Validator.Phone.NotViable.Log"))
        return result
      }
      var isPossible = PhoneUtil.isPossibleNumberWithReason(phoneNumber, phoneCountry)
      switch (isPossible) {
        case GWPhoneValidationResult.INVALID:
          result.addError(DisplayKey.get("Validator.Phone.Invalid.Log", phoneCountry))
          break
        case GWPhoneValidationResult.INVALID_COUNTRY_CODE:
          result.addError(DisplayKey.get("Validator.Phone.InvalidCountryCode.Log", phoneCountry))
          break
        case GWPhoneValidationResult.TOO_LONG:
          result.addError(DisplayKey.get("Validator.Phone.TooLong.Log", phoneCountry))
          break
        case GWPhoneValidationResult.TOO_SHORT:
          result.addError(DisplayKey.get("Validator.Phone.TooShort.Log", phoneCountry))
          break
        case GWPhoneValidationResult.IS_POSSIBLE:
        default:
          if (!PhoneUtil.isValidNumber(phoneNumber, phoneCountry)) {
            if (strict) {
              result.addError(DisplayKey.get("Validator.Phone.Invalid.Log", phoneCountry))
            }
          }

...
uses gw.plugin.pcintegration.PCExtraFieldIntegrationUtil
uses gw.plugin.policy.search.IPolicySearchAdapter
uses wsi.remote.gw.webservice.pc.pc5000.ccpolicysearchintegration.CCPolicySearchIntegration
uses wsi.remote.gw.webservice.pc.pc5000.entities.types.complex.CCPCFilteringCriteria
...
class PolicySearchPCPlugin implements IPolicySearchAdapter, InitializablePlugin {
  ...
  private function retrievePolicy( policyNumber : String, lossDate: Date, pcFilter : CCPCFilteringCriteria ) : PolicyRetrievalResultSet   {
    ...
    try {
      ...
      PCExtraFieldIntegrationUtil.remapAllExtraFields(env)

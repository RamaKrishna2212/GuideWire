...
uses jakarta.activation.DataSource
uses jakarta.mail.Authenticator
uses jakarta.mail.MessagingException
uses jakarta.mail.PasswordAuthentication
uses jakarta.mail.SendFailedException
uses jakarta.mail.Session
uses jakarta.mail.internet.InternetAddress
...
@PluginParameter(:name="mail[.].*", :helpText = "This will pass any parameter starting with \"mail.\" to jakarta.mail.Session.getInstance")
...
class JavaxEmailMessageTransport implements MessageTransport, InitializablePlugin {
  ...
  private function handleInvalidAddresses(email : Email, invalidAddresses : jakarta.mail.Address[]) : boolean {

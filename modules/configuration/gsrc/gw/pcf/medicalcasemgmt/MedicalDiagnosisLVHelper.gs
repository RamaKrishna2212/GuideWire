...
class MedicalDiagnosisLVHelper {
  ...
  function validateICDCode(code: String): String {
    return validateICDCode(code, _injury)
  }

  function validateICDCode(code: String, injury: InjuryIncident): String {
    if (injury != null and injury.InjuryDiagnoses.where(\i -> i.ICDCode.Code == code).Count > 1) {

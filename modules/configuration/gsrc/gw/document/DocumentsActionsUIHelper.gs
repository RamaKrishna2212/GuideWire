...
class DocumentsActionsUIHelper {
  ...
  var _documentStoreSuspended : Boolean = null
  ...
  ***********************************************************************************/
  ...
  property get DocumentStoreSuspended () : boolean {
    if(_documentStoreSuspended == null) {
      var isSuspended = false
      try {
        var destStatus = MessagingUtil.getDestinationStatus(324)
        isSuspended = MessagingUtil.isSuspendedState(destStatus)
      } catch (ex : Exception) {
        CCLoggerCategory.DOCUMENT.error("Error retrieving destination status for destination 324", ex);
      }
      _documentStoreSuspended = isSuspended
    }
    return _documentStoreSuspended
  }
  ...
  ***********************************************************************************/
  ...
  private function isDocumentEditable(document : Document) : boolean {
    return document != null
        ...
        and (document.Status != DocumentStatusType.TC_FINAL or PLConfigParameters.FinalDocumentsNotEditable.Value == false)
  }

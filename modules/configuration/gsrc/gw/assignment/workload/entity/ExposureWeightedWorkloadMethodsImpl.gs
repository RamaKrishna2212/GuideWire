...
public class ExposureWeightedWorkloadMethodsImpl extends AbstractWeightedWorkloadMethodsBaseImpl<Exposure> {
  ...
  public override function assignUserByWorkload(includeSubgroups : boolean, withinGroup : Group) : boolean {
    ...
    var result : boolean
    CCLoggerCategory.ASSIGNMENT_WEIGHTED_WORKLOAD.debug("assignUserbyWorkload: attempting to assign Exposure " + Owner.LogDisplayName)
    result = super.assignUserByWorkload(includeSubgroups, withinGroup)
    CCLoggerCategory.ASSIGNMENT_WEIGHTED_WORKLOAD.debug("assignUserbyWorkload: new assignee: " + Owner.AssignedUser?.Credential.LogDisplayName)
    return result
  }
  ...
  public override function assignUserByWorkloadAndAttributes(crit : AttributeBasedAssignmentCriteria, includeSubGroups : boolean, withinGroup : Group) : boolean {
    ...
    if (result) {
      CCLoggerCategory.ASSIGNMENT_WEIGHTED_WORKLOAD.debug("assignUserbyWorkload: successfully assigned to: " + Owner.AssignedUser?.Credential.LogDisplayName)
    }
    return result
  }

package gw.webservice.cc.cc5040.contact
...
uses gw.webservice.contactapi.ab5040.ABClientAPI
uses gw.webservice.contactapi.ab5040.ABClientAPIAddressBookUIDContainer
uses gw.webservice.contactapi.ab5040.ABClientAPIPendingChangeContext
 ...
 */
@Export
@gw.xml.ws.annotation.WsiWebService( "http://guidewire.com/cc/ws/gw/webservice/cc/cc5040/contact/ContactAPI" )
...
class ContactAPI implements ABClientAPI {
  ...
  private function createRejectionActivity(context: ABClientAPIPendingChangeContext, pattern: ActivityPattern ){
    ...
    if (selectedClaim == null || selectedClaim.isClosed()) {
      return
    }
  ...
  private function appendInstanceChanges(instance: XmlBackedInstance, objectPath:String):String {
    ...
    for (field in instance.Field) {
      if (not(isExcludedField(field)) and not java.util.Objects.deepEquals(field.OrigValue, field.Value)) {
    ...
    for (fkItem in instance.Fk) {
      ...
      if (not java.util.Objects.deepEquals(fkItem.OrigValue, fkInstance.LinkID)) {  // if the FK has changed where it points
  ...
  protected function generateMapFromABUIDContainer(addressBookUIDContainer:ABClientAPIAddressBookUIDContainer) : Map<String,String> {
    ...
    for (tuple in addressBookUIDContainer.AddressBookUIDTuples) {
      var entityName = ContactIntegrationMapperFactory.mapper().NameMapper.getLocalEntityName(tuple.EntityType)

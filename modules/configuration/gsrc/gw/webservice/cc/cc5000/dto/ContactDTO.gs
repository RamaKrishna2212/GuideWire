package gw.webservice.cc.cc5000.dto
 ...
 * A(n) {@link gw.webservice.cc.cc5000.dto.ContactDTO} is a DTO that represents an instance of {@link Contact} for use by certain WS-I Web Services in the base product. This file is composed of basic getters and setters, plus members that make it easier for server-side code to work with DTOs, like virtual getters that query for entities by PublicID, and methods to copy fields to or from an instance of {@link Contact}.
...
final class ContactDTO {
  ...
  var _pronounAggregate           : PronounAggregateType          as PronounAggregate
  var _pronounObjectiveText       : String                        as PronounObjectiveText
  var _pronounPossessiveText      : String                        as PronounPossessiveText
  var _pronounSubjectiveText      : String                        as PronounSubjectiveText
  var _publicID                   : String                        as PublicID
  var _score                      : Integer                       as Score
  var _sexAssignedAtBirth         : SexAssignedAtBirthType        as SexAssignedAtBirth
  ...
  final function readFrom(that : Contact) : ContactDTO {
    ...
    if (that typeis Person) {
      ...
      Prefix                      = that.Prefix
      PronounAggregate            = that.PronounAggregate
      PronounObjectiveText        = that.PronounObjectiveText
      PronounPossessiveText       = that.PronounPossessiveText
      PronounSubjectiveText       = that.PronounSubjectiveText
      SexAssignedAtBirth          = that.SexAssignedAtBirth
      Suffix                      = that.Suffix
   ...
   */
  reified function writeTo<T extends Contact>(that : T, ignoreNullValues = true) : T {
    // if field is on base class
    ...
    if (that typeis Person) {
      ...
      if (PronounAggregate           != null or !ignoreNullValues) that.PronounAggregate           = PronounAggregate
      if (PronounObjectiveText       != null or !ignoreNullValues) that.PronounObjectiveText       = PronounObjectiveText
      if (PronounPossessiveText      != null or !ignoreNullValues) that.PronounPossessiveText      = PronounPossessiveText
      if (PronounSubjectiveText      != null or !ignoreNullValues) that.PronounSubjectiveText      = PronounSubjectiveText
      if (SexAssignedAtBirth         != null or !ignoreNullValues) that.SexAssignedAtBirth         = SexAssignedAtBirth
   ...
   * @param bundleProvider The entity in whose Bundle to create the new Contact.
   ...
   */
  reified function writeToNewEntityIn<T extends Contact>(bundleProvider: BundleProvider, createNew: block(): T = null, ignoreNullValues = true): T {
    var instance: T

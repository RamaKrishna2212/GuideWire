package gw.webservice.cc.cc5000.financials
...
uses gw.currency.fxrate.CCDefaultFXRateBuilder
uses gw.pl.persistence.core.Bundle
uses gw.pl.util.ArgCheck
uses gw.plugin.approval.ApprovalResult
uses gw.transaction.Transaction
uses gw.webservice.cc.cc5000.CCWsiEntityApiBase
uses gw.webservice.cc.cc5000.dto.ActivityDTO
uses gw.webservice.cc.cc5000.dto.CheckDTO
uses gw.webservice.cc.cc5000.dto.CheckPayeeDTO
uses gw.webservice.cc.cc5000.dto.DocumentDTO
uses gw.webservice.cc.cc5000.dto.TransactionDTO
uses gw.webservice.cc.cc5000.dto.TransactionLineItemDTO
uses gw.webservice.cc.cc5000.dto.TransactionSetDTO
...
@WsiWebService("http://guidewire.com/cc/ws/gw/webservice/cc/cc5000/financials/ClaimFinancialsAPI")
...
class ClaimFinancialsAPI extends CCWsiEntityApiBase {
  ...
  public function createCheckSet(transactionSetDTO : TransactionSetDTO, checkForDuplicateChecks : boolean) : TransactionSetApprovalResult {
    ...
    for (check in transactionSetDTO.NewChecks) {
      check.NewPayments.each( \ elt -> verifyTransactionStatusNotSpecified(elt))
    }
  ...
  private function assembleCheckSet(checkSetDTO: TransactionSetDTO, bundle: Bundle) : CheckCreator {
    ...
    if (exchangeRateProvided) {
      if (check.isUsingDeprecatedExchangeRates()) {
        ...
        final var transToReservingExchangeRateInUse = new TransToReservingExchangeRateInUse(bundle)
        transToReservingExchangeRateInUse.initializeFrom(new CCDefaultFXRateBuilder()
            .withMarketAsManual(check.Currency, check.ReservingCurrency, checkSetDTO.NewExchangeRate).build());
        check.TransToReservingExchangeRateInUse = transToReservingExchangeRateInUse
      }
    }

    var checkSet = check.CheckSet
    checkSetDTO.writeTo(checkSet)
    checkSet.CreatedVia = CreatedVia.TC_WEB_SERVICE
  ...
  private function assembleRecoverySet(recoverySetDTO : TransactionSetDTO, bundle : Bundle) : RecoverySet {
    return CCCurrencyUtil.getUseDeprecatedExchangeRates()
        ? assembleRecoverySetLegacy(recoverySetDTO, bundle)
        : assembleRecoverySetFXRates(recoverySetDTO, bundle)
  }

  private function assembleRecoverySetLegacy(recoverySetDTO : TransactionSetDTO, bundle : Bundle) : RecoverySet {
  ...
  private function assembleRecoverySetFXRates(recoverySetDTO : TransactionSetDTO, bundle : Bundle) : RecoverySet {
    var claim = bundle.add(recoverySetDTO.Claim)
    ...
    newRecoveries.each( \ recoveryDTO -> {
      ...
      // Process exchange rate, if specified
      if (exchangeRateProvided) {
        final var customTransToReservingExchangeRate = new TransToReservingExchangeRateInUse(bundle)
        customTransToReservingExchangeRate.initializeFrom(new CCDefaultFXRateBuilder()
                .withMarketAsManual(recovery.Currency, recovery.ReservingCurrency, recoverySetDTO.NewExchangeRate).build());
        recovery.TransToReservingExchangeRateInUse = customTransToReservingExchangeRate
      }
    })
  ...
  private function assembleRecoveryReserveSet(recoveryReserveSetDTO : TransactionSetDTO, bundle : Bundle)
      : RecoveryReserveSet {
    return CCCurrencyUtil.getUseDeprecatedExchangeRates()
        ? assembleRecoveryReserveSetLegacy(recoveryReserveSetDTO, bundle)
        : assembleRecoveryReserveSetFXRates(recoveryReserveSetDTO, bundle)
  }

  private function assembleRecoveryReserveSetLegacy(recoveryReserveSetDTO : TransactionSetDTO, bundle : Bundle)
      : RecoveryReserveSet {
  ...
  private function assembleRecoveryReserveSetFXRates(recoveryReserveSetDTO : TransactionSetDTO, bundle : Bundle)
      : RecoveryReserveSet {
    var claim = bundle.add(recoveryReserveSetDTO.Claim)
    ...
    newRecoveryReserves.eachWithIndex( \ recoveryReserveDTO, index -> {
      ...
      if (exchangeRateProvided) {
        final var customTransToReservingExchangeRate = new TransToReservingExchangeRateInUse(bundle)
        customTransToReservingExchangeRate.initializeFrom(new CCDefaultFXRateBuilder()
                .withMarketAsManual(recoveryReserve.Currency, recoveryReserve.ReservingCurrency, recoveryReserveSetDTO.NewExchangeRate).build());
        recoveryReserve.TransToReservingExchangeRateInUse = customTransToReservingExchangeRate
      }
    })
  ...
  private function assembleReserveSet(reserveSetDTO : TransactionSetDTO, bundle : Bundle) : ReserveSet {
    return CCCurrencyUtil.getUseDeprecatedExchangeRates()
        ? assembleReserveSetLegacy(reserveSetDTO, bundle)
        : assembleReserveSetFXRates(reserveSetDTO, bundle)
  }

  private function assembleReserveSetLegacy(reserveSetDTO : TransactionSetDTO, bundle : Bundle) : ReserveSet {
    var claim = bundle.add(reserveSetDTO.Claim)

    var reserveSet = claim.newReserveSet()
    reserveSetDTO.writeTo(reserveSet)
    reserveSet.CreatedVia = CreatedVia.TC_WEB_SERVICE
  ...
  private function assembleReserveSetFXRates(reserveSetDTO : TransactionSetDTO, bundle : Bundle) : ReserveSet {
    var claim = bundle.add(reserveSetDTO.Claim)
    ...
    reserveSetDTO.NewReserves.each( \ reserveDTO -> {
      ...
      if (exchangeRateProvided) {
        final var customTransToReservingExchangeRate = new TransToReservingExchangeRateInUse(bundle)
        customTransToReservingExchangeRate.initializeFrom(new CCDefaultFXRateBuilder()
                .withMarketAsManual(reserve.Currency, reserve.ReservingCurrency, reserveSetDTO.NewExchangeRate).build());
        reserve.TransToReservingExchangeRateInUse = customTransToReservingExchangeRate
      }
    })

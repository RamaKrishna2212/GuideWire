package gw.webservice.cc.cc5000.financials.bulkpay
...
uses gw.webservice.cc.cc5000.dto.BIValidationAlertDTO
uses gw.webservice.cc.cc5000.dto.BulkInvoiceDTO
uses gw.webservice.cc.cc5000.dto.BulkInvoiceItemDTO
uses gw.webservice.cc.cc5000.dto.BulkInvoiceItemInfoDTO
...
@WsiWebService("http://guidewire.com/cc/ws/gw/webservice/cc/cc5000/financials/bulkpay/BulkInvoiceAPI")
...
class BulkInvoiceAPI extends CCWsiEntityApiBase {
  ...
  public function createBulkInvoice(bulkInvoiceDTO : BulkInvoiceDTO) : String {
    ...
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      if (customExchangeRate != null) {
        ...
        if (bulkInvoice.isUsingDeprecatedExchangeRates()) {
          ...
          var bulkInvoiceExchangeRateInUse = new BulkInvoiceExchangeRateInUse(bundle)
          bulkInvoiceExchangeRateInUse.initializeFrom(new CCDefaultFXRateBuilder()
                  .withMarketAsManual(bulkInvoice.Currency, CurrencyUtil.ReportingCurrency, customExchangeRate)
                  .build())
          bulkInvoice.TransToReportingExchangeRateInUse = bulkInvoiceExchangeRateInUse
        }
  ...
  * <br/>
   ...
   * <code>requestBulkInvoice()</code> causes the <code>BulkInvoice</code> to move to the <code>BulkInvoiceStatus</code>
   * of <code>TC_INITIATINGESCALATION</code>. Work queues will eventually move it and its associated checks to the
   * <code>BulkInvoiceStatus</code> of <code>TC_REQUESTING</code>.
   * If the scheduled send date is set to a future date, then the invoice is unaffected.
   * </p>
   *
  ...
  public function requestBulkInvoice(invoiceID : String) {
    Transaction.runWithNewBundle(\ bundle -> {
      ...
      bulkInvoice.requestAsynchronously()
    })
  }

  /**
   * <br/>
   * Sets the status of the <code>BulkInvoice</code> and all its items to <code>TC_INITIATINGVOID</code>. Also voids any
  ...
  public function voidBulkInvoice(invoiceID : String) {
    Transaction.runWithNewBundle(\ bundle -> {
      fetchByPublicID<BulkInvoice>(invoiceID, bundle).voidAsynchronously()
    })
  }

  /**
   * <br/>
   * Sets the status of the <code>BulkInvoice</code> and all its items to <code>TC_INITIATINGSTOP</code>. Also stops any
   * bulk checks associated with the items.
   * <p/><br/>
   * Note : This API can be used only on <code>BulkInvoices</code> that are currently having a status of :
   * <ul>
  ...
  public function stopBulkInvoice(invoiceID : String) {
    Transaction.runWithNewBundle(\ bundle -> {
      fetchByPublicID<BulkInvoice>(invoiceID, bundle).stopAsynchronously()
    })
  }

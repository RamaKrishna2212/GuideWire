...
class InstantCheckUtil {
  ...
  public static function getPaymentMethods(check : Check, hasPayeeAccountInfo : boolean) : List<PaymentMethod> {
    var methods = new ArrayList<PaymentMethod>()
    if (allowInstantCheck(check)) {
      methods.addAll(PaymentMethod.TF_PERSONCHECKPAYEE.TypeKeys)
    } else {
      methods.addAll(PaymentMethod.TF_NONPERSONCHECKPAYEE.TypeKeys)
    }

    if (hasPayeeAccountInfo and check.Payees.length <= 1) {
      methods.add(PaymentMethod.TC_ACCOUNT)
    }
    return methods
  }
